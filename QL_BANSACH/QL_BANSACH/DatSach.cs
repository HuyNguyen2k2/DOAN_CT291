...
namespace QL_BANSACH
{
    public partial class DatSach : Form
    {
        ...
        private void DisplayImage(string image)
        {
            ...
            string imagePath = Environment.CurrentDirectory + "\\imgs\\" + image;
                ...
                && !string.IsNullOrEmpty(txtSL.Text) && !string.IsNullOrEmpty(txtGia.Text))
            {
                try
                {
                    using (SqlConnection connection = new SqlConnection(chuoiketnoi))
                    {
                        ...
                        if(isCreated == 0)
                        {
                        string queryDonDatHang = "INSERT INTO DON_DAT_HANG (MADDH, MAKH, MANV, NGAYDAT, DIACHI, SDT) VALUES (@MADDH, @MAKH, @MANV, @NGAYDAT, @DIACHI, @SDT)";
